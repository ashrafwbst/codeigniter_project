...
class Carousel extends CI_Controller {
    ...
    public function index() {
        ...
        // Pages variable
        $result_per_page = 20;
        $total_row = $this->Cms_model->countData('carousel_widget');
        $num_link = 10;
        $base_url = $this->Cms_model->base_link(). '/admin/carousel/';
        ...
        $this->template->setSub('carousel', $this->Cms_admin_model->getIndexData('carousel_widget', $result_per_page, $pagination, 'timestamp_create', 'desc'));
    ...
    public function insert() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            //Validation passed
            //Add the user
            $this->Cms_admin_model->insertCarousel();
            //Return to user list
            $this->db->cache_delete_all();
            $this->Cms_model->clear_file_cache('carousel_*', TRUE);
    ...
    public function editPhoto() {
        ...
        //Load the form helper
        $this->load->helper('form');
        $this->cms_referrer->setIndex('carousel_edit'); /* Set index page when redirect after save */
        if ($this->uri->segment(4)) {
            $carousel = $this->Cms_model->getValue('*', 'carousel_widget', 'carousel_widget_id', $this->uri->segment(4), 1);
            if($carousel !== FALSE){
                $this->template->setSub('carousel', $carousel);
                $this->load->library('pagination');            
                $search_arr = "carousel_widget_id = '".$this->uri->segment(4)."'";
                // Pages variable           
                $total_row = $this->Cms_model->countData('carousel_picture', $search_arr);           
                $this->template->setSub('showfile', $this->Cms_admin_model->getIndexData('carousel_picture', 0, 0, 'arrange', 'ASC', $search_arr));
                $this->template->setSub('total_row', $total_row);
                //Load the view
    ...
    public function update() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if($this->uri->segment(4)){
                //Update the user
                $this->Cms_admin_model->updateCarousel($this->uri->segment(4));
                //Return to user list
                $this->db->cache_delete_all();
                $this->Cms_model->clear_file_cache('carousel_*', TRUE);
                $this->session->set_flashdata('error_message', '<div class="alert alert-success" role="alert">' . $this->lang->line('success_message_alert') . '</div>');
    ...
    public function addYoutube() {
        ...
        if ($this->uri->segment(4)) {
            ...
            if ($youtube_url) {
                $this->Cms_admin_model->insertCarouselUpload($this->uri->segment(4), $carousel_type, '', $youtube_url);
                $this->db->cache_delete_all();
            }                
            $this->session->set_flashdata('error_message', '<div class="alert alert-success" role="alert">' . $this->lang->line('success_message_alert') . '</div>');
            redirect($this->cms_referrer->getIndex('carousel_edit'), 'refresh');
        } else {
    ...
    public function addUrl() {
        ...
        if ($this->uri->segment(4)) {
            ...
            if ($photo_url) {
                $this->Cms_admin_model->insertCarouselUpload($this->uri->segment(4), $carousel_type, '', '', $photo_url);
                $this->db->cache_delete_all();
            }                
            $this->session->set_flashdata('error_message', '<div class="alert alert-success" role="alert">' . $this->lang->line('success_message_alert') . '</div>');
            redirect($this->cms_referrer->getIndex('carousel_edit'), 'refresh');
        ...
        if ($this->uri->segment(4) && !empty($_FILES['files'])) {
            ...
            for ($i = 0; $i < $cpt; $i++) {
                if ($files['files']['name'][$i]) {
                    ...
                    $file_id1 = $this->Cms_admin_model->file_upload($photo, $photo_name, '', $path, $file_id, '');
                    if ($file_id1) {
                        $this->Cms_admin_model->insertCarouselUpload($this->uri->segment(4), $carousel_type, $file_id1);
                    }
                }
            }
            $this->db->cache_delete_all();
    ...
    public function filesSave() {
        ...
        if (isset($filedel)) {
            ...
            foreach ($filedel as $value) {
                if ($value) {
                    $filename = $this->Cms_model->getValue('file_upload', 'carousel_picture', 'carousel_picture_id', $value, 1);
                    if ($filename->file_upload) {
                        @unlink($path . $filename->file_upload);
                    }
                    $this->Cms_admin_model->removeData('carousel_picture', 'carousel_picture_id', $value);
                }
            }
        }
        ...
        if(isset($delR)){
            foreach ($delR as $delV) {
                if ($delV) {
                    $path = FCPATH . "/photo/carousel/";
                    //Delete the data
                    $filedel = $this->Cms_model->getValue('*', 'carousel_picture', 'carousel_widget_id', $delV);
                    if (!empty($filedel)) {
                        foreach ($filedel as $value) {
                            if ($value) {
                                ...
                                $this->Cms_admin_model->removeData('carousel_picture', 'carousel_picture_id', $value->carousel_picture_id);
                            }
                        }
                    }
                    $this->Cms_admin_model->removeData('carousel_widget', 'carousel_widget_id', $delV);
                }
            }
        }
        $this->db->cache_delete_all();
