...
class Export extends CI_Controller {
    ...
    public function index() {
        ...
        $this->cms_referrer->setIndex('export');
    ...
    public function getCSV() {
        ...
        if($this->uri->segment(4) && $this->db->table_exists($this->uri->segment(4))){
            $this->Cms_admin_model->exportCSV($this->uri->segment(4), $this->uri->segment(4), $this->input->get('fieldS'), '', $this->input->get('orderby', TRUE), $this->input->get('sort', TRUE));
        }else{
            redirect($this->cms_referrer->getIndex('export'), 'refresh');
        }
    }
        ...
        if($this->uri->segment(4) && $this->db->table_exists($this->uri->segment(4))){
            ...
            if (file_exists($newfname)) {
                ...
                if ($csv_array) {
                    ...
                    $this->template->setSub('primary_key', $this->Cms_admin_model->findPrimaryKey($this->uri->segment(4)));
    ...
    public function importDB() {
        ...
        if($this->uri->segment(4) && $this->db->table_exists($this->uri->segment(4))){
            ...
            $primary_key = $this->Cms_admin_model->findPrimaryKey($this->uri->segment(4));
            ...
            if ($csv_array) {
                ...
                foreach ($csv_array as $row) {
                    $data_row = array();
                    foreach ($row as $key => $value) {
                        $dbfield = $this->security->xss_clean($field[$this->Cms_model->cleanEmailFormat($key)]);
                        if ($dbfield) {
