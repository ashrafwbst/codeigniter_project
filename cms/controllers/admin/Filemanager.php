...
class Filemanager extends CI_Controller {
    ...
    public function _init() {
        ...
        $this->template->set('core_js', $this->Cms_admin_model->coreJs('//cdnjs.cloudflare.com/ajax/libs/require.js/2.3.5/require.min.js'));
    ...
    public function index() {
        ...
        $this->cms_referrer->setIndex('filemanager');
        ...
        if($this->Cms_auth_model->is_group_allowed('delete', 'backend') !== FALSE){
            $connect_url = $this->Cms_model->base_link() . "/admin/filemanager/connector/";
        }else{
            $connect_url = $this->Cms_model->base_link() . "/admin/filemanager/connectorNodel/";
        }
    ...
    private function cmsConfigAdmin() {
        if($this->Cms_auth_model->is_in_group(1)){
            return array(
                    'driver' => 'LocalFileSystem',
                    'path' => FCPATH,
                    'URL' => '',
                    'alias' => 'CMS System Config',
    ...
    public function connector() {
        ...
        $opts = array(
            'roots' => array(
                ...
                array(
                    'driver' => 'LocalFileSystem',
                    'path' => FCPATH . 'cms/views/templates',
                    'URL' => '',
                    'alias' => 'Template Main',
                    'trashHash' => 't1_Lw', // elFinder's hash of trash folder
                    'winHashFix' => DIRECTORY_SEPARATOR !== '/', // to make hash same to Linux one on windows too
                ...
                array(
                    'driver' => 'LocalFileSystem',
                    'path' => FCPATH . 'cms/modules/plugin/views/templates',
                    'URL' => '',
                ...
                array(
                    'driver' => 'LocalFileSystem',
                    'path' => FCPATH . 'cms/views/frontpage/templates',
                    'URL' => '',
    ...
    public function templateCopy() {
        ...
        if (is_dir($template_assets_path . $template_name) || is_dir($template_main_path . $template_name) || is_dir($template_plugin_path . $template_name) || is_dir($template_morefront_path . $template_name)) {
            ...
            redirect($this->cms_referrer->getIndex('filemanager'), 'refresh');
            exit(1);
        }
        $this->Cms_model->copy_recursive($template_assets_path . 'default', $template_assets_path . $template_name);
        $this->Cms_model->copy_recursive($template_main_path . 'default', $template_main_path . $template_name);
        $this->Cms_model->copy_recursive($template_plugin_path . 'default', $template_plugin_path . $template_name);
        $this->Cms_model->copy_recursive($template_morefront_path . 'default', $template_morefront_path . $template_name);
        if (file_exists($template_assets_path . $template_name . DIRECTORY_SEPARATOR . 'css' . DIRECTORY_SEPARATOR . 'default.min.css') && file_exists($template_assets_path . $template_name . DIRECTORY_SEPARATOR . 'css' . DIRECTORY_SEPARATOR . 'default.css')) {
            @rename($template_assets_path . $template_name . DIRECTORY_SEPARATOR . 'css' . DIRECTORY_SEPARATOR . 'default.min.css', $template_assets_path . $template_name . DIRECTORY_SEPARATOR . 'css' . DIRECTORY_SEPARATOR . $template_name . '.min.css');
            @rename($template_assets_path . $template_name . DIRECTORY_SEPARATOR . 'css' . DIRECTORY_SEPARATOR . 'default.css', $template_assets_path . $template_name . DIRECTORY_SEPARATOR . 'css' . DIRECTORY_SEPARATOR . $template_name . '.css');
            $path_to_main = $template_main_path . $template_name . DIRECTORY_SEPARATOR . 'main.php';
            $main_contents = @file_get_contents($path_to_main);
            $main_contents1 = str_replace("default", $template_name, $main_contents);
