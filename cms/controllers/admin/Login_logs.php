...
class Login_logs extends CI_Controller {
    ...
    public function index() {
        ...
        $result_per_page = 50;
        $total_row = $this->Cms_admin_model->countTable('login_logs', $search_arr);
        $num_link = 10;
        $base_url = $this->Cms_model->base_link(). '/admin/loginlogs/';
        // Pageination config
        $this->Cms_admin_model->pageSetting($base_url,$total_row,$result_per_page,$num_link); 
        ($this->uri->segment(3))? $pagination = ($this->uri->segment(3)) : $pagination = 0;
        //Get users from database
        $this->template->setSub('login_logs', $this->Cms_admin_model->getIndexData('login_logs', $result_per_page, $pagination, 'timestamp_create', 'desc', $search_arr));
    ...
    public function deleteLoginLogs() {
        ...
        if(isset($delR)){
            foreach ($delR as $value) {
                if ($value) {
                    $this->Cms_admin_model->removeData('login_logs', 'login_logs_id', $value);
                }
            }
        }
    ...
    public function settings() {
        ...
        $this->cms_referrer->setIndex();
        $this->load->helper('form');
        $this->template->setSub('settings', $this->Cms_model->load_bf_config());
        $this->template->setSub('whitelist', $this->Cms_model->getValueArray('*', 'whitelist_ip', '', '', 0, ' 	timestamp_create', 'desc'));
        $this->template->setSub('blacklist', $this->Cms_model->getValueArray('*', 'blacklist_ip', '', '', 0, ' 	timestamp_create', 'desc'));
    ...
    public function settings_save() {
        ...
        $this->Cms_admin_model->updateBFSettings();
    ...
    public function whiteipsave() {
        ...
        admin_helper::is_allowchk('protection settings');
        admin_helper::is_allowchk('save');
        $this->Cms_admin_model->saveWhiteIP();
        $this->session->set_flashdata('error_message','<div class="alert alert-success" role="alert">'.$this->lang->line('success_message_alert').'</div>');
        redirect($this->cms_referrer->getIndex(), 'refresh');
    }
    ...
    public function blackipsave() {
        ...
        admin_helper::is_allowchk('protection settings');
        admin_helper::is_allowchk('save');
        $this->Cms_admin_model->saveBlackIP();
        $this->session->set_flashdata('error_message','<div class="alert alert-success" role="alert">'.$this->lang->line('success_message_alert').'</div>');
        redirect($this->cms_referrer->getIndex(), 'refresh');
    }
    ...
    public function whiteipdel() {
        ...
        if($this->uri->segment(4)){
            $this->Cms_admin_model->removeData('whitelist_ip', 'whitelist_ip_id', $this->uri->segment(4));
            $this->db->cache_delete_all();
        }
        $this->session->set_flashdata('error_message','<div class="alert alert-success" role="alert">'.$this->lang->line('success_message_alert').'</div>');
        redirect($this->cms_referrer->getIndex(), 'refresh');
    ...
    public function blackipdel() {
        ...
        admin_helper::is_allowchk('delete');
        if($this->uri->segment(4)){
            $this->Cms_admin_model->removeData('blacklist_ip', 'blacklist_ip_id', $this->uri->segment(4));
            $this->db->cache_delete_all();
        }
        $this->session->set_flashdata('error_message','<div class="alert alert-success" role="alert">'.$this->lang->line('success_message_alert').'</div>');
        redirect($this->cms_referrer->getIndex(), 'refresh');
    ...
    public function genPrivateKey() {
        ...
        admin_helper::is_allowchk('protection settings');
        admin_helper::is_allowchk('save');
        $this->Cms_admin_model->makePrivateKey();
        $this->db->cache_delete_all();
        $this->session->set_flashdata('error_message','<div class="alert alert-success" role="alert">'.$this->lang->line('success_message_alert').'</div>');
        redirect($this->cms_referrer->getIndex(), 'refresh');
    }
