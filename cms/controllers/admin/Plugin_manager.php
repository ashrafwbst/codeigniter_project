...
class Plugin_manager extends CI_Controller {
    ...
    public function index() {
        ...
        $total_row = $this->Cms_model->countData('plugin_manager');
        //Get users from database
        $this->template->setSub('plugin_mgr', $this->Cms_model->getValueArray('*', 'plugin_manager', '', ''));
        $this->template->setSub('total_row', $total_row);
        $xml_data = $this->Cms_admin_model->getPluginXML('', 'filename', $this->input->get('search', TRUE));
        // Pages variable
        $result_per_page = 20;
        if($xml_data !== FALSE){
        ...
        $num_link = 10;
        $base_url = $this->Cms_model->base_link(). '/admin/plugin/';

        // Pageination config
        $this->Cms_admin_model->pageSetting($base_url, $total_xml, $result_per_page, $num_link);
        ($this->uri->segment(3)) ? $pagination = ($this->uri->segment(3)) : $pagination = 0;
        $this->template->setSub('plugin_list', $this->Cms_admin_model->getIndexDataFromObj($xml_data, $result_per_page, $pagination));
    ...
    public function setstatus() {
        ...
        if ($this->uri->segment(4)) {
            $status = $this->Cms_model->getValue('plugin_active', 'plugin_manager', "plugin_config_filename != '' AND plugin_manager_id = '".$this->uri->segment(4)."'", '', 1);
    ...
    public function install() {
        ...
        if ($this->uri->segment(4) && $this->Cms_admin_model->chkPluginInst($this->uri->segment(4)) === FALSE) {
            $version = $this->Cms_admin_model->pluginLatestVer($this->uri->segment(4));
            $url1 = $this->config->item('cms_plugin_install_server_1') . $this->uri->segment(4)."-install-" . $version . ".zip";
            $url2 = $this->config->item('cms_plugin_install_server_2') . $this->uri->segment(4)."-install-" . $version . ".zip";
            if($this->Cms_model->is_url_exist($url1) !== FALSE){
                $url = &$url1; /* Main Link */
            }else if($this->Cms_model->is_url_exist($url1) === FALSE && $this->Cms_model->is_url_exist($url2) !== FALSE){
                $url = &$url2; /* Backup Link */
            }
            $newfname = FCPATH . basename($url);
            if($this->Cms_model->downloadFile($url, $newfname) !== FALSE){
    ...
    public function upgrade() {
        ...
        if($this->Cms_admin_model->chkVerUpdate($this->Cms_model->getVersion()) !== FALSE){
            ...
            redirect($this->Cms_model->base_link().'/admin/upgrade', 'refresh');
        }
        if (function_exists('ini_set')) {
            @ini_set('max_execution_time', 600);
            @ini_set('memory_limit','512M');
        ...
        if ($this->uri->segment(4) && $this->Cms_admin_model->chkPluginInst($this->uri->segment(4)) !== FALSE) {
            $this->Cms_model->clear_all_cache();
            $last_ver = $this->Cms_admin_model->pluginLatestVer($this->uri->segment(4));
            $cur_ver = $this->Cms_model->getPluginConfig($this->uri->segment(4), 'plugin_version');
            if($this->Cms_admin_model->chkPluginUpdate($cur_ver, $last_ver) !== FALSE){
                $nextversion = $this->Cms_admin_model->pluginNextVer($cur_ver, $last_ver);
                $url1 = $this->config->item('cms_plugin_upgrade_server_1') . $this->uri->segment(4)."-upgrade-" . $nextversion . ".zip";
                $url2 = $this->config->item('cms_plugin_upgrade_server_2') . $this->uri->segment(4)."-upgrade-" . $nextversion . ".zip";
                if($this->Cms_model->is_url_exist($url1) !== FALSE){
                    $url = &$url1; /* Main Link */
                }else if($this->Cms_model->is_url_exist($url1) === FALSE && $this->Cms_model->is_url_exist($url2) !== FALSE){
                    $url = &$url2; /* Backup Link */
                ...
                if($this->Cms_model->downloadFile($url, $newfname) !== FALSE){
                    ...
                    if($this->Cms_admin_model->chkPluginUpdate($this->Cms_model->getPluginConfig($this->uri->segment(4), 'plugin_version'), $last_ver) !== FALSE){
    ...
    public function uninstall() {
        ...
        if ($this->uri->segment(4)) {
            if($this->Cms_admin_model->pluginUninstall($this->uri->segment(4)) !== FALSE){
                $this->db->cache_delete_all();
                $this->session->set_flashdata('error_message', '<div class="alert alert-success" role="alert">' . $this->lang->line('success_message_alert') . '</div>');
                redirect($this->cms_referrer->getIndex(), 'refresh');
            }else{
