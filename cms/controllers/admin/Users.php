...
class Users extends CI_Controller {
    ...
    public function index() {
        ...
        // Pages variable
        $result_per_page = 20;
        $total_row = $this->Cms_model->countData('user_admin', $search_arr);
        $num_link = 10;
        $base_url = $this->Cms_model->base_link(). '/admin/users/';
        // Pageination config
        $this->Cms_admin_model->pageSetting($base_url,$total_row,$result_per_page,$num_link);     
        ($this->uri->segment(3))? $pagination = $this->uri->segment(3) : $pagination = 0;
        //Get users from database
        $this->template->setSub('users', $this->Cms_admin_model->getIndexData('user_admin', $result_per_page, $pagination, 'user_admin_id', 'asc', $search_arr));        
    ...
    public function addUser() {
        ...
        $this->template->setSub('group', $this->Cms_auth_model->get_group_all());
        //Load the view
    ...
    public function confirm() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            //Validation passed
            //Add the user
            $this->Cms_admin_model->createUser();
    ...
    public function editUser() {
        ...
        if($this->uri->segment(4)){
            ...
            $users = $this->Cms_admin_model->getUser($this->uri->segment(4), 'admin');
            if($users !== FALSE){
    ...
    public function edited() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            //Validation passed
            //Update the user
            $rs = $this->Cms_admin_model->updateUser($this->uri->segment(4));
            if($rs !== FALSE){
                //Return to user list
                $this->db->cache_delete_all();
                $this->session->set_flashdata('error_message','<div class="alert alert-success" role="alert">'.$this->lang->line('success_message_alert').'</div>');
    ...
    public function delete() {
        ...
        if($this->uri->segment(4)){
            if ($this->session->userdata('user_admin_id') != $this->uri->segment(4)) {
                ...
                $this->Cms_admin_model->removeUser($this->uri->segment(4));
                $this->Cms_auth_model->remove_user_from_allgroup($this->uri->segment(4));
                $this->db->cache_delete_all();
                $this->session->set_flashdata('error_message','<div class="alert alert-success" role="alert">'.$this->lang->line('success_message_alert').'</div>');
            } else {
    ...
    public function forgot() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $to_email = $email;
            $message_html = $this->lang->line('email_dear').$email.',<br><br>'.$this->lang->line('email_reset_message').'<br><a href="'.$this->Cms_model->base_link().'/admin/reset/'.$md5_hash.'" target="_blank"><b>'.$this->Cms_model->base_link().'/admin/reset/'.$md5_hash.'</b></a><br><br>'.$this->lang->line('email_footer').'<br><a href="'.$this->Cms_model->base_link().'" target="_blank"><b>'.$row->site_name.'</b></a>';
