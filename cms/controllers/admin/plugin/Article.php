...
class Article extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->lang->load('plugin/article', $this->Cms_admin_model->getLang());
    ...
    public function index() {
        ...
        $this->cms_referrer->setIndex('article'); /* Set index page when redirect after save */
        $this->db->cache_on();
        //Get users from database
        $this->template->setSub('total_cat', $this->Cms_model->countData('article_db', "active = '1' AND is_category = '1'"));
        $this->template->setSub('total_art', $this->Cms_model->countData('article_db', "active = '1' AND is_category = '0'"));
    ...
    public function article() {
        admin_helper::is_logged_in($this->session->userdata('admin_email'));
        admin_helper::is_allowchk('article');
        $this->cms_referrer->setIndex('article_art'); /* Set index page when redirect after save */
        $this->db->cache_on();
        $search_arr = ' 1=1 ';
        ...
        $num_link = 10;
        $base_url = $this->Cms_model->base_link(). '/admin/plugin/article/article/';
        ...
        $this->template->setSub('category', $this->Cms_model->getValueArray('*', 'article_db', "is_category", '1', 0, 'arrange', 'asc'));
    ...
    public function category() {
        admin_helper::is_logged_in($this->session->userdata('admin_email'));
        admin_helper::is_allowchk('article');
        $this->cms_referrer->setIndex('article_cat'); /* Set index page when redirect after save */
        $this->db->cache_on();
        ...
        //Get users from database
        $this->template->setSub('category', $this->Cms_model->getValueArray('*', 'article_db', $search_arr, '', 0, 'arrange', 'asc'));
        $this->template->setSub('main_category', $this->Cms_model->getValueArray('*', 'article_db', "is_category = '1' AND main_cat_id = ''", '', 0, 'arrange', 'asc'));
        $this->template->setSub('total_row', $total_row);
        $this->template->setSub('lang', $this->Cms_model->loadAllLang());
    ...
    public function catIndexSave(){
        ...
        redirect($this->cms_referrer->getIndex('article_cat'), 'refresh');
    }
    ...
    public function catadd() {
        ...
        //Load the form helper
        $this->load->helper('form');
        $this->template->setSub('category', $this->Cms_model->getValueArray('*', 'article_db', "is_category = '1' AND main_cat_id = ''", '', 0, 'arrange', 'asc'));
        $this->template->setSub('lang', $this->Cms_model->loadAllLang());
        //Load the view
    ...
    public function addSave() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->Cms_model->clear_file_cache('article_getWidget_*', TRUE);
            $this->db->cache_delete_all();
            redirect($this->cms_referrer->getIndex('article_art'), 'refresh');
        }
    }
    ...
    public function artedit() {
        ...
        if ($this->uri->segment(5)) {
            $this->template->setSub('category', $this->Cms_model->getValueArray('*', 'article_db', "is_category", '1', 0, 'arrange', 'asc'));
            $this->template->setSub('article', $this->Cms_model->getValue('*', 'article_db', 'article_db_id', $this->uri->segment(5), 1));
            $this->template->setSub('lang', $this->Cms_model->loadAllLang());
            //Load the view
    ...
    public function editArtSave() {
        ...
        if ($this->uri->segment(5)) {
            ...
            redirect($this->cms_referrer->getIndex('article'), 'refresh');
        }
    }
    ...
    public function catedit() {
        ...
        if ($this->uri->segment(5)) {
            $this->template->setSub('main_category', $this->Cms_model->getValueArray('*', 'article_db', "is_category = '1' AND main_cat_id = ''", '', 0, 'arrange', 'asc'));
            $this->template->setSub('category', $this->Cms_model->getValue('*', 'article_db', 'article_db_id', $this->uri->segment(5), 1));
            $this->template->setSub('lang', $this->Cms_model->loadAllLang());
            //Load the view
    ...
    public function asCopy() {
        ...
        admin_helper::is_allowchk('save');
        if($this->uri->segment(5)){
            $article = $this->Cms_model->getValue('*', 'article_db', "article_db_id = '".$this->uri->segment(5)."' AND is_category = '0'", '', 1);
            if($article !== FALSE){
                $data = array(
                    'main_picture' => '',
                    'title' => $this->Cms_model->findNameAsCopy('article_db', 'article_db_id', $article->title),
                    'url_rewrite' => $this->Cms_model->findNameAsCopy('article_db', 'article_db_id', $article->url_rewrite, TRUE),
                ...
                $this->Cms_model->insertAsCopy('article_db', $data);
                $this->db->cache_delete_all();
            }
        }
        redirect($this->cms_referrer->getIndex('article_art'), 'refresh');
