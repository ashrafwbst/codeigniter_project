...
class Template {
    ...
    function loadFrontPlugin($view = '', $othermainfile = '')
    {
        ...
        if(file_exists(FCPATH . 'cms/modules/plugin/views/templates/'.$config->themes_config.'/'.$view.'.php') !== FALSE){
            return $this->load('templates/'.$config->themes_config.'/'.$view, $this->data_sub, '', '', $othermainfile);
        }else{
            return $this->load('templates/default/'.$view, $this->data_sub, '', '', $othermainfile);
        }
    }
    ...
    {   
        $CI =& get_instance();
        $config = $CI->Cms_model->load_config();
        if(file_exists(FCPATH . 'cms/views/frontpage/templates/'.$config->themes_config.'/'.$view.'.php') !== FALSE){
            return $this->load('frontpage/templates/'.$config->themes_config.'/'.$view, $this->data_sub, '', '', $othermainfile);
        }else{
            return $this->load('frontpage/templates/default/'.$view, $this->data_sub, '', '', $othermainfile);
        }
    }
