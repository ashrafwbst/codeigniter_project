...
class Cms_admin_model extends CI_Model{
    ...
    public function load_config(){
        return $this->Cms_model->load_config_admin();
    }
   ...
   /**
     ...
     */
    public function setAdminLang3($lang_iso = '') {
        $this->db->set('admin_lang', $lang_iso);
    ...
    public function chkVerUpdate($cur_ver, $xml_url = ''){
        ...
        if($last_ver){
            return $this->Cms_model->chkVerUpdate($cur_ver, $last_ver);
        }else{
            return FALSE;
        }
    }
    ...
    public function findNextVersion($cur_txt, $xml_url = ''){
        $last_ver = $this->getLatestVersion($xml_url);
        if($last_ver){
            return $this->Cms_model->findNextVersion($cur_txt, $last_ver);
        }else{
            return FALSE;
        }
    }
    ...
    public function countTable($table, $search_sql = ''){
        ...
        return $this->Cms_model->countData($table, $search_sql);
    }
    ...
    public function getIndexData($table, $limit = 0, $offset = 0, $orderby = '', $sort = '', $search_sql = '', $groupby = '', $join_db = '', $join_where = '', $join_type = '', $sel_field = '*'){
        ...
        $count = $this->Cms_model->countData($table, $search_sql, $groupby, $orderby, $sort, $join_db, $join_where, $join_type);
     ...
     * @return  array
     */
    public function getIndexData_paging($table, $limit = 0, $offset = 0, $orderby = '', $sort = '', $search_sql = '', $groupby = '', $join_db = '', $join_where = '', $join_type = '', $sel_field = '*'){
        // Get a list of all user accounts
        $count = $this->Cms_model->countData($table, $search_sql, $groupby, $orderby, $sort, $join_db, $join_where, $join_type);
        $this->db->select($sel_field);
        if($join_db && $join_where){
    ...
    public function getUser($id, $type = ''){
        ...
        $rows = $this->Cms_model->getValue('*', 'user_admin', $sql_where, '', 1);
        if($rows !== FALSE){
            return $rows;
        }else{
            return FALSE;
    ...
    public function getUserEmail($id){
        ...
        if (!$this->cache->get('getUserEmail_'.$id)) {
            $rows = $this->Cms_model->getValue('email', 'user_admin', "user_admin_id", $id, 1);
            if($rows !== FALSE){
                $return = $rows->email;
            }else{
                $return = FALSE;
    ...
    public function createUser($member = FALSE){
        ...
        $data = array(
            ...
            'password' => $this->Cms_model->pwdEncypt($this->input->post('password', TRUE)),
    ...
    public function updateUser($id){
        $query = $this->Cms_model->chkPassword($this->session->userdata('admin_email'), $this->input->post('cur_password', TRUE), "user_type != 'member'");
        if($query['num_rows'] !== 0){
            ...
            if($this->input->post('password') != ''){
                $this->db->set('password', $this->Cms_model->pwdEncypt($this->input->post('password', TRUE)), TRUE);
            ...
            if($id != 1 && $this->session->userdata('user_admin_id') != $id){
                ...
                $count = $this->Cms_model->countData('user_to_group', $data);
    ...
    public function removeDataAndFile($table, $id_field, $id_val, $file_path, $file_sql_field){
        if($table && $id_field && $file_path && $file_sql_field){
            $file = $this->Cms_model->getValue($file_sql_field, $table, $id_field, $id_val, 1);
    ...
    public function sessionLoginChk(){
        ...
        if($this->session->userdata('session_id')){
            $numcount = $this->Cms_model->countData('user_admin', "session_id = '".$this->session->userdata('session_id')."' AND email = '".$this->session->userdata('admin_email')."'");
     ...
     *
     */
    public function cmsCopyright(){
        /* Please do not remove or change this function */
        $cms_copyright = '<br><span class="copyright">'.$this->cmsCredit().'</span>';
    ...
    public function coreCss($more_css = '', $more_include = TRUE){
        $core_css = '<link rel="canonical" href="' . $this->Cms_model->base_link(). '/' . $this->uri->uri_string() . '" />' . "\n";
    ...
    public function coreMetatags($desc_txt, $more_metatags = ''){
        ...
        $meta = array(
            ...
            array('name' => 'generator', 'content' => $this->generateMeta()),
    ...
    public function updateSocial(){
        ...
        $this->Cms_model->clear_file_cache('getsocial*', TRUE);
    }

    public function getSocialValue($social_name='')
    {
        $get = $this->db->get_where('footer_social', array('social_name' => $social_name))->row_array();
        return $get['social_url'];
    }
    ...
    public function file_upload($photo, $photo_name1, $tmp_photo, $uploaddir, $photo_id, $paramiter, $yearR = '', $maxSizeR = 1900, $resize = TRUE, $limitsize = 100){
		 ...
		 */
                if ($this->Cms_model->photo_xss_clean($photo) === FALSE) {
    ...
    public function getAllMenu($drop_page_menu_id = 0, $lang = '', $position = 0){
        return $this->Cms_model->main_menu($drop_page_menu_id, $lang, $position, TRUE);
    }
    ...
    public function updateMenu($id){
        ...
        $this->db->set('other_link', $other_link, TRUE);
    ...
    public function findLangDataUpdate($lang_iso){
        ...
        if($query->num_rows() !== 0){
            foreach($query->result() as $rows){
                $this->db->set('lang_iso', $this->Cms_model->getDefualtLang(), TRUE);
                $this->db->set('timestamp_update', 'NOW()', FALSE);
    ...
    public function insertLang(){
        $arrange = $this->Cms_model->getLastID('lang_iso', 'arrange');
        ($this->input->post('active')) ? $active = $this->input->post('active', TRUE) : $active = 0;
        $data = array(
    ...
    public function updateLang($id){
        $old_lang = $this->Cms_model->getValue('lang_iso', 'lang_iso', 'lang_iso_id', $id, 1);
        $this->load->dbforge();
    ...
    public function chkPageName($page_name_input){
        if($page_name_input == 'assets' || $page_name_input == 'cms' ||
    ...
    public function insertPage(){
        ...
        ($this->input->post('active')) ? $active = $this->input->post('active', TRUE) : $active = 0;
        $page_url = $this->chkPageName($this->Cms_model->rw_link($page_name_input));
        ...
        if($this->Cms_auth_model->is_group_allowed('pages cssjs additional', 'backend') !== FALSE){
    ...
    public function updateForms($id){
                ...
                if($field_oldname[$i] && $form_field_id[$i]){
                    ...
                    if($field_oldname[$i] != $field_name1[$i]){
                        if($field_oldtype[$i] != 'button' && $field_oldtype[$i] != 'reset' && $field_oldtype[$i] != 'submit' && $field_oldtype[$i] != 'label' && $field_oldtype[$i] != 'file'){
                            ...
                            delete_files(FCPATH . "/photo/forms/".$this->Cms_model->cleanEmailFormat($form_name).'/'.$this->Cms_model->cleanEmailFormat($field_oldname[$i]), TRUE);
                            delete_files(FCPATH . "/photo/forms/".$this->Cms_model->cleanEmailFormat($form_name).'/'.$this->Cms_model->cleanEmailFormat($field_oldname[$i]));
                            rmdir(FCPATH . "/photo/forms/".$this->Cms_model->cleanEmailFormat($form_name).'/'.$this->Cms_model->cleanEmailFormat($field_oldname[$i]));
        ...
        $field_required = $this->input->post('field_required', TRUE);
        if(count($field_name) > 0){
            $last_arrange = $this->Cms_model->getLastID('form_field', 'arrange', "form_main_id = '".$id."'");
            for($i = 0; $i < count($field_name); $i++){
                if($field_name[$i]){
                    $last_arrange++;
                    $data = array(
    ...
    public function chkPluginActive($plugin_config_filename){
        if($plugin_config_filename){
            $status = $this->Cms_model->getValue('plugin_active', 'plugin_manager', "plugin_config_filename", $plugin_config_filename, 1);
    ...
    public function updateWidget($id){
        ...
        $this->Cms_model->clear_file_cache('widget_'.$this->Cms_model->encodeURL($this->input->post("widget_name", TRUE)));
    }
     ...
     * updateBFSettings
     *
     * Function for update the Security settings
     */
    ...
    public function saveWhiteIP(){
        ...
        if($this->Cms_model->chkBFwhitelistIP($ip_address) === FALSE && !empty($ip_address)){
            $data = array(
    ...
    public function setMaintenance() {
        ...
        $this->db->update('settings', $data);
        $this->db->cache_delete_all();
        $this->Cms_model->clear_file_cache('config');
    ...
    public function getSaveDraftJS() {
        ...
        $(document).ready(function () {
            $("#save_draft").click(function () {
                ...
                var urlpost = "'.$this->Cms_model->base_link().'/admin/admin/saveDraft'.'";
                $.ajax({
                    url: urlpost,
                    type: "POST",
                    data: fields,
    ...
    public function getPluginXML($xml_url = '', $key_search = '', $search = ''){       
        if(!$xml_url) $xml_url = $this->config->item('cms_pluginxmlurl_main');
        if (!$this->cache->get('plugin_list_xml' . md5($xml_url))) {
            $xml_url_bak = $this->config->item('cms_pluginxmlurl_backup');
            if($this->Cms_model->is_url_exist($xml_url) !== FALSE){
                $return = $this->Cms_model->get_contents_url($xml_url);
            }else if($this->Cms_model->is_url_exist($xml_url) === FALSE && $this->Cms_model->is_url_exist($xml_url_bak) !== FALSE){
                $return = $this->Cms_model->get_contents_url($xml_url_bak);
            }else{
                $return = FALSE;
            }
            ($this->load_config()->pagecache_time == 0) ? $cache_time = 1 : $cache_time = $this->load_config()->pagecache_time;
        ...
        if($this->cache->get('plugin_list_xml' . md5($xml_url)) !== FALSE && $key_search && $search){
            ...
            $plugin = $xml->xpath("//cms/plugin/" . $key_search . "[.='$search']/parent::*");
    ...
    public function chkPluginInst($plugin_config_filename){
        $count = $this->Cms_model->countData('plugin_manager', "plugin_config_filename = '".$plugin_config_filename."'");        
        if($count != 0){
    ...
    private function cmsCredit(){

        return $this->config->item('cms_credit').' Version '.$this->Cms_model->getVersion();
    }
    
    public function generateMeta(){

        return 'CMS | Version '.$this->Cms_model->getVersion();
    }
    ...
    public function pluginUninstall($plugin_config_filename){
        $plugin_file_path = $this->Cms_model->getPluginConfig($plugin_config_filename, 'plugin_file_path');
        $plugin_db_table = $this->Cms_model->getPluginConfig($plugin_config_filename, 'plugin_db_table');
        if($plugin_file_path !== FALSE && is_array($plugin_file_path) && $plugin_config_filename != 'article' && $plugin_config_filename != 'gallery'){
            foreach ($plugin_file_path as $value) {
                if($value && is_file($value) && is_writable($value)){
                    ...
                    $this->Cms_model->rmdir_recursive($value);
                }
            }
            ...
            $this->removeData('user_perms', "name LIKE '".$this->Cms_model->getPluginConfig($plugin_config_filename, 'plugin_name')."%'");
   ...
   public function dirSize($directory, $dept = 0) {
    ...
    public function insertCarouselUpload($carousel_widget_id, $carousel_type, $fileupload = '', $youtube_url = '', $photo_url = '') {
        $img_rs = $this->Cms_model->getValue('arrange', 'carousel_picture', "carousel_widget_id", $carousel_widget_id, 1, 'arrange', 'desc');
        if(!empty($img_rs)){
    ...
    public function updatePWidget($id){
        ...
        $this->Cms_model->clear_file_cache('pwidget_' . md5($id));
    }
     ...
     * Function for create the new user
     */
    public function createProduct($member = FALSE){
        error_reporting(1);
        ...
        $extra_images=array();
        $uploadData='';
        //$tot_xtra_img=count($_FILES['extra_images']['name']);
        //print_r($_FILES['extra_images']['type']);
        /*for($i=0;$i<$tot_xtra_img;$i++){
            if(!empty($_FILES['extra_images'][$i]) && $_FILES['extra_images']['type'][$i] == 'image/png' || $_FILES['extra_images']['type'][$i] == 'image/jpg' || $_FILES['extra_images']['type'][$i] == 'image/jpeg' || $_FILES['extra_images']['type'][$i] == 'image/gif'){

            $paramiter = $i;
            $photo_id = time();
            $uploaddir = 'photo/products/';
            $file_f = $_FILES['extra_images']['tmp_name'][$i];
            $file_name = $_FILES['extra_images']['name'][$i];
            $extra_images[] = $this->file_upload($file_f, $file_name, '', $uploaddir, $photo_id, $paramiter);
              }
             }*/
              //echo '<pre>';print_r($extra_images);die;

       
        if(!empty($_FILES['product_image']) && $_FILES['product_image']['type'] == 'image/png' || $_FILES['product_image']['type'] == 'image/jpg' || $_FILES['product_image']['type'] == 'image/jpeg' || $_FILES['product_image']['type'] == 'image/gif'){
            $paramiter = '_1';
            $photo_id = time();
            $uploaddir = 'photo/products/';
            $file_f = $_FILES['product_image']['tmp_name'];
        ...
        foreach ($extra_images as $extra_image) {
         ...
         $this->db->insert('products_extra_images', $data);
        }
      }
    ...
    public function addExtraImg(){

        //print_r($_POST);
        //print_r($_FILES);die;
        if(!empty($_FILES['prod_sub_img']) && $_FILES['prod_sub_img']['type'] == 'image/png' || $_FILES['prod_sub_img']['type'] == 'image/jpg' || $_FILES['prod_sub_img']['type'] == 'image/jpeg' || $_FILES['prod_sub_img']['type'] == 'image/gif'){
            $paramiter = '_1';
            $photo_id = time();
            $uploaddir = 'photo/products/';
            $file_f = $_FILES['prod_sub_img']['tmp_name'];
            $file_name = $_FILES['prod_sub_img']['name'];
            $upload_file = $this->file_upload($file_f, $file_name, '', $uploaddir, $photo_id, $paramiter);
       ...
       public function getProdExtraimg($id){
        $this->db->select('*'); 
       $this->db->from('products_extra_images');
    ...
    public function updateProduct($id){
       ...
       /* for($i=0;$i<$tot_xtra_img;$i++){
        ...
        $upload_file = $this->input->post('product_image');
        if(!empty($_FILES['product_image']) && $_FILES['product_image']['type'] == 'image/png' || $_FILES['product_image']['type'] == 'image/jpg' || $_FILES['product_image']['type'] == 'image/jpeg' || $_FILES['product_image']['type'] == 'image/gif'){
            $paramiter = '_1';
            $photo_id = time();
            $uploaddir = 'photo/products/';
       ...
       public function removeProduct($id){
        $this->db->where('product_id',$id);
        $this->db->delete('products_extra_images');
        $this->db->where('id',$id);
       ...
       public function createProductCategory(){
        $upload_file = '';
        if(!empty($_FILES['category_image']) && $_FILES['category_image']['type'] == 'image/png' || $_FILES['category_image']['type'] == 'image/jpg' || $_FILES['category_image']['type'] == 'image/jpeg' || $_FILES['category_image']['type'] == 'image/gif'){
            $paramiter = '_1';
            $photo_id = time();
            $uploaddir = 'photo/products/category';
            $file_f = $_FILES['category_image']['tmp_name'];
       ...
       public function get_prod_categories(){
         $query = $this->db->get('product_category');  
            return $query->result_array();  
       }

       public function get_prod_subcategories(){
         $query = $this->db->get('product_category');  
            return $query->result_array();  
       }
       public function productCategoies(){
        $this->db->select('*');
     ...
     public function productsubCategoies(){
        $this->db->select('*');
        $this->db->where('id !=',0,FALSE);
       ...
       public function getsubProdCategory($id){
       $this->db->select('*'); 
       $this->db->from('product_subcategory');
       $this->db->where('id',$id);
       $query=$this->db->get();
    ...
    public function updateProductCategory($id){
        $upload_file = $this->input->post('category_image', TRUE);
        if(!empty($_FILES['category_image']) && $_FILES['category_image']['type'] == 'image/png' || $_FILES['category_image']['type'] == 'image/jpg' || $_FILES['category_image']['type'] == 'image/jpeg' || $_FILES['category_image']['type'] == 'image/gif'){
            $paramiter = '_1';
            $photo_id = time();
            $uploaddir = 'photo/products/category';
       ...
       $this->db->update('product_category',$data);
        ...
        public function updateProductsubCategory($id){
        $upload_file = $this->input->post('category_image', TRUE);
        if(!empty($_FILES['category_image']) && $_FILES['category_image']['type'] == 'image/png' || $_FILES['category_image']['type'] == 'image/jpg' || $_FILES['category_image']['type'] == 'image/jpeg' || $_FILES['category_image']['type'] == 'image/gif'){
            $paramiter = '_1';
            $photo_id = time();
    ...
    public function removeProductCat($id){
        $this->db->set('product_category',0);
...
public function removeProductsubCat($id){
        $this->db->set('product_subcategory',0);
    ...
    {
      return  $this->db->get('our_services_content')->row_array();
    }

     public function getAllClients()
    {
      return  $this->db->get('our_clients')->result_array();
    }

    public function getAllPartners()
    {
      return  $this->db->get('our_partners')->row_array();
    }

    public function getAllContents()
    {
      return  $this->db->get('content_manager')->result_array();
    }

    public function getPageName($page_id)
    {
       $getPageName = $this->db->get_where('page_menu', array('page_menu_id' => $page_id))->row_array();
       return $getPageName['menu_name'];
    }

    public function getMenus()
    {
       return $this->db->get_where('page_menu',array('active' => 1))->result_array();
    }
   
   
    public function insertAlbum()
    {
        //  if(!empty($_FILES['album_image']) && $_FILES['album_image']['type'] == 'image/png' || $_FILES['album_image']['type'] == 'image/jpg' || $_FILES['album_image']['type'] == 'image/jpeg' || $_FILES['album_image']['type'] == 'image/gif'){
        //     $paramiter = '_1';
        //     $photo_id = time();
        //     $uploaddir = 'photo/album/';
        //     $file_f = $_FILES['album_image']['tmp_name'];
        //     $file_name = $_FILES['album_image']['name'];
        //     $upload_album_file = $this->file_upload($file_f, $file_name, '', $uploaddir, $photo_id, $paramiter);
        // }

          $pdf_one = '';
        if(!empty($_FILES['pdf_one'])){
            $paramiter = '_1';
            $photo_id = time();
            $uploaddir = 'photo/album/one';
            $file_f = $_FILES['pdf_one']['tmp_name'];
            $file_name = $_FILES['pdf_one']['name'];
            $pdf_one = $this->file_upload($file_f, $file_name, '', $uploaddir, $photo_id, $paramiter);
        } 

           $pdf_two = '';
        if(!empty($_FILES['pdf_two'])){
            $paramiter = '_1';
            $photo_id = time();
            $uploaddir = 'photo/album/two';
            $file_f = $_FILES['pdf_two']['tmp_name'];
            $file_name = $_FILES['pdf_two']['name'];
            $pdf_two = $this->file_upload($file_f, $file_name, '', $uploaddir, $photo_id, $paramiter);
        } 

        $data = array(
                'menu_id' => $this->input->post('menu_id'),
                'heading' => strtoupper($this->input->post('heading', TRUE)),
                'content' => $this->input->post('content', TRUE),
              //  'image' => $upload_album_file,
                'pdf_one' =>  $pdf_one,
                'pdf_two' =>  $pdf_two,
                'status' => 1,
    ...
    public function deletAlbum($album_id)
    {
     ...
     public function deletSubAlbum($sub_album_id)
    {
        $this->db->where('id', $sub_album_id);
        $this->db->delete('sub_album_images');
        if($this->db->affected_rows() > 0)
        {
            return 1;
        }else
    ...
    public function deletClientImg($clients_id='')
    {
  ...
  public function getAlbum($ablum_id='')
    {
        
        return $this->db->get_where('album', array("id" => $ablum_id))->row_array();
    }

 
    public function getAlbums($menu_id='')
    {
        
        return $this->db->get_where('album', array("menu_id" => $menu_id))->result_array();
    }


    public function getContentFrontend($menu_id='')
    {
        return $this->db->get_where('content_manager', array("menu_id" => $menu_id))->row_array();
    }
    ...
    public function getAllSubAlbums($album_id=''){
    return $this->db->get_where('sub_album_images', array("album_id" => $album_id))->result_array();
    ...
    public function insertSubAlbum()
    {
        ...
        $album_id = $this->input->post('album_id');
           ...
           $data = array(
                'album_id' => $album_id,
                'sub_image' => $upload_album_file,
                'status' => 1,
                'add_date'    => date('Y-m-d H:i:a')
                );
      
       $this->db->insert('sub_album_images', $data);
       if($this->db->affected_rows() > 0)
       {
        return 1;
       }else
      ...
      public function insertClientImg()
    {
         if(!empty($_FILES['album_image']) && $_FILES['album_image']['type'] == 'image/png' || $_FILES['album_image']['type'] == 'image/jpg' || $_FILES['album_image']['type'] == 'image/jpeg' || $_FILES['album_image']['type'] == 'image/gif'){
            $paramiter = '_1';
            $photo_id = time();
            $uploaddir = 'photo/clients';
            $file_f = $_FILES['album_image']['tmp_name'];
            $file_name = $_FILES['album_image']['name'];
            $upload_album_file = $this->file_upload($file_f, $file_name, '', $uploaddir, $photo_id, $paramiter);
        }
    ...
    public function updateSubAlbum($subAlbum_id='')
    {
         if(!empty($_FILES['album_image']) && $_FILES['album_image']['type'] == 'image/png' || $_FILES['album_image']['type'] == 'image/jpg' || $_FILES['album_image']['type'] == 'image/jpeg' || $_FILES['album_image']['type'] == 'image/gif'){
            $paramiter = '_1';
            $photo_id = time();
        ...
        $this->db->where('id', $subAlbum_id);
       $this->db->update('sub_album_images', $data);
       if($this->db->affected_rows() > 0)
       {
        return 1;
       }else
    ...
    public function getSubAlbum($sub_album_id="")
    {
         return $this->db->get_where('sub_album_images', array("id" => $sub_album_id))->row_array();
    }

     public function getContentByID($content_id="")
    {
         return $this->db->get_where('content_manager', array("id" => $content_id))->row_array();
    }
    ...
    {
        $upload_album_file_one = '';
        $upload_album_file_two = '';
        $upload_album_file_three = '';
        $upload_album_file_four = '';
        $background_image = '';
        if(!empty($_FILES['background_image']) && $_FILES['background_image']['type'] == 'image/png' || $_FILES['background_image']['type'] == 'image/jpg' || $_FILES['background_image']['type'] == 'image/jpeg' || $_FILES['background_image']['type'] == 'image/gif'){
            $paramiter = '_1';
            $photo_id = time().uniqid();
            $uploaddir = 'photo/partner/';
            $file_f = $_FILES['background_image']['tmp_name'];
            $file_name = $_FILES['background_image']['name'];
            $background_image = $this->file_upload($file_f, $file_name, '', $uploaddir, $photo_id, $paramiter);
        }
         if(!empty($_FILES['image_one']) && $_FILES['image_one']['type'] == 'image/png' || $_FILES['image_one']['type'] == 'image/jpg' || $_FILES['image_one']['type'] == 'image/jpeg' || $_FILES['image_one']['type'] == 'image/gif'){
            $paramiter = '_1';
            $photo_id = time().uniqid();
            $uploaddir = 'photo/partner/';
            $file_f = $_FILES['image_one']['tmp_name'];
            $file_name = $_FILES['image_one']['name'];
            $upload_album_file_one = $this->file_upload($file_f, $file_name, '', $uploaddir, $photo_id, $paramiter);
        }
        if(!empty($_FILES['image_two']) && $_FILES['image_two']['type'] == 'image/png' || $_FILES['image_two']['type'] == 'image/jpg' || $_FILES['image_two']['type'] == 'image/jpeg' || $_FILES['image_two']['type'] == 'image/gif'){
            $paramiter = '_1';
            $photo_id = time().uniqid();
            $uploaddir = 'photo/partner/';
            $file_f = $_FILES['image_two']['tmp_name'];
            $file_name = $_FILES['image_two']['name'];
            $upload_album_file_two = $this->file_upload($file_f, $file_name, '', $uploaddir, $photo_id, $paramiter);
        }
        if(!empty($_FILES['image_three']) && $_FILES['image_three']['type'] == 'image/png' || $_FILES['image_three']['type'] == 'image/jpg' || $_FILES['image_three']['type'] == 'image/jpeg' || $_FILES['image_three']['type'] == 'image/gif'){

            $paramiter = '_1';
            $photo_id = time().uniqid();
            $uploaddir = 'photo/partner/';
            $file_f = $_FILES['image_three']['tmp_name'];
            $file_name = $_FILES['image_three']['name'];
            $upload_album_file_three = $this->file_upload($file_f, $file_name, '', $uploaddir, $photo_id, $paramiter);
        }

         if(!empty($_FILES['image_four']) && $_FILES['image_four']['type'] == 'image/png' || $_FILES['image_four']['type'] == 'image/jpg' || $_FILES['image_four']['type'] == 'image/jpeg' || $_FILES['image_four']['type'] == 'image/gif'){

            $paramiter = '_1';
            $photo_id = time().uniqid();
            $uploaddir = 'photo/partner/';
            $file_f = $_FILES['image_four']['tmp_name'];
            $file_name = $_FILES['image_four']['name'];
            $upload_album_file_four = $this->file_upload($file_f, $file_name, '', $uploaddir, $photo_id, $paramiter);
        }

       
        $data = array(
                'heading' => strtoupper($this->input->post('heading', TRUE)),
                'sub_heading' =>  $this->input->post('content', TRUE),
                'image_one' => $upload_album_file_one,
                'image_two' => $upload_album_file_two,
                'image_three' => $upload_album_file_three,
                'image_four' => $upload_album_file_four,
                'background_image' => $background_image,
                'status' => 1,
                'add_date'    => date('Y-m-d H:i:a')
                );

            if(empty($upload_album_file_one))
            {
                unset($data['image_one']);
            }
             if(empty($upload_album_file_two))
            {
                unset($data['image_two']);
            }
            if(empty($upload_album_file_three))
            {
                unset($data['image_three']);
            }
            if(empty($upload_album_file_four))
            {
    ...
    public function updateServices()
    {
        ...
        $data = array(
                ...
                'image_one_subheading' =>   $this->input->post('image_one_subheading', TRUE),
                'image_two_heading' =>  $this->input->post('image_two_heading', TRUE),
                'image_two_subheading' =>   $this->input->post('image_two_subheading', TRUE),
                'image_three_heading' =>  $this->input->post('image_three_heading', TRUE),
                'image_three_subheading' =>   $this->input->post('image_three_subheading', TRUE),
                'image_one' => $upload_album_file_one,
                'image_two' => $upload_album_file_two,
                'image_three' => $upload_album_file_three,
                'status' => 1,
     ...
     //  print_r($data); exit;
       ...
       $this->db->update('our_services_content', $data);
       if($this->db->affected_rows() > 0)
       {
        return 1;
       }else
    ...
    public function insertContent()
    {
        ...
        $menu_id = $this->input->post('menu_id');
        $image_heading = $this->input->post('image_heading');
        ...
        $status = $this->input->post('status'); 
           ...
           $data = array(
                'banner_image' => $upload_album_file_one,
       ...
       $this->db->insert('content_manager', $data);
       if($this->db->affected_rows() > 0)
       {
        return 1;
       }else
  ...
  public function insertblogmanager()
    {
         if(!empty($_FILES['blog_image']) && $_FILES['blog_image']['type'] == 'image/png' || $_FILES['blog_image']['type'] == 'image/jpg' || $_FILES['blog_image']['type'] == 'image/jpeg' || $_FILES['blog_image']['type'] == 'image/gif'){
            $paramiter = '_1';
            $photo_id = time();
            $uploaddir = 'photo/blog/';
            $file_f = $_FILES['blog_image']['tmp_name'];
            $file_name = $_FILES['blog_image']['name'];
            $upload_blog_file = $this->file_upload($file_f, $file_name, '', $uploaddir, $photo_id, $paramiter);
        }

          $data = array(
                'heading' => strtoupper($this->input->post('heading', TRUE)),
                'subheading'=>strtoupper($this->input->post('subheading', TRUE)),
                'content' => $this->input->post('content', TRUE),
                'image' => $upload_blog_file,
                'status' => 1,
                'add_date'    => date('Y-m-d')
                );
      
       $this->db->insert('blog_manager', $data);
       if($this->db->affected_rows() > 0)
       {
        return 1;
       }else
  ...
  public function deleteblogmanager($blogmanager_id)
    {
        $this->db->where('id', $blogmanager_id);
        $this->db->delete('blog_manager');
        if($this->db->affected_rows() > 0)
        {
            return 1;
        }else
   ...
   public function getblogmanager($blogmanager_id='')
    {
        
        return $this->db->get_where('blog_manager', array("id" => $blogmanager_id))->row_array();
    }

 
    

    public function updateblogmanager($blogmanager_id = '')
    {
         if(!empty($_FILES['blog_image']) && $_FILES['blog_image']['type'] == 'image/png' || $_FILES['blog_image']['type'] == 'image/jpg' || $_FILES['blog_image']['type'] == 'image/jpeg' || $_FILES['blog_image']['type'] == 'image/gif'){
            $paramiter = '_1';
            $photo_id = time();
       ...
       $this->db->where('id', $blogmanager_id);
       $this->db->update('blog_manager', $data);
       if($this->db->affected_rows() > 0)
       {
        return 1;
       }else
    ...
    {
      return  $this->db->get('blog_manager')->result_array();
    }
}
