...
class Cms_auth_model extends CI_Model {
    ...
    public function get_group_id($group_parm) {
        if (is_numeric($group_parm)) {
            ...
            return $this->Cms_model->getID('user_groups', 'user_groups_id', "name = '" . $group_parm . "'");
        }
    }
    ...
    public function get_group_all($sortby = '') {
        if($sortby){
            return $this->Cms_admin_model->getIndexData('user_groups', 0, 0, $sortby);
        }else{
            return $this->Cms_admin_model->getIndexData('user_groups', 0, 0, 'user_groups_id', 'asc');
        }
    }
    ...
    public function get_perms_all($type) {
        if($type){
            return $this->Cms_admin_model->getIndexData('user_perms', 0, 0, 'user_perms_id', 'asc', "permstype = '".$type."'");
        }else{
            return FALSE;
        }
    }
    ...
    public function get_perm_id($perm_parm, $type = '') {
        if (is_numeric($perm_parm)) {
            ...
            return $this->Cms_model->getID('user_perms', 'user_perms_id', "name = '" . $perm_parm . "' AND permstype = '".$type."'");
        }
    }
    ...
    public function delete_group($user_groups_id) {
        if($user_groups_id != 1 && $user_groups_id != 2 && $user_groups_id != 3 && $user_groups_id != 4){
            $this->Cms_admin_model->removeData('user_to_group', 'user_groups_id', $user_groups_id);
            $this->Cms_admin_model->removeData('user_perm_to_group', 'user_groups_id', $user_groups_id);
            $this->Cms_admin_model->removeData('user_groups', 'user_groups_id', $user_groups_id);
        }
    }
    ...
    public function remove_user_group($user_admin_id, $group_parm) {
        ...
        $this->Cms_admin_model->removeData('user_to_group', $data);
    }
    ...
    public function remove_user_from_allgroup($user_admin_id) {
        $this->Cms_admin_model->removeData('user_to_group', array('user_admin_id' => $user_admin_id));
    }
    ...
    public function is_useractive($user_admin_id = '') {
        ...
        $count = $this->Cms_model->countData('user_admin', $data);
        if ($count !== FALSE && $count !== 0) {
            return TRUE;
        } else {
            return FALSE;
    ...
    public function is_group_allowed($perm_par, $type = '', $group_par = '') {
        ...
        if ($group_par) {
            ...
            $count = $this->Cms_model->countData('user_perm_to_group', $data);
    ...
    public function deny_group($group_par, $perm_par, $type = '') {
        ...
        $this->Cms_admin_model->removeData('user_perm_to_group', $data);
    }
    ...
    public function send_pm($receiver_ids, $title, $message, $sender_id = '') {
        ...
        if ($sender_id && (!$this->is_useractive($sender_id))) {
            ...
            if ($receiver_ids && is_numeric($receiver_ids) && $sender_id != $receiver_ids) {
                ...
                $sender_user = $this->Cms_admin_model->getUser($sender_id);
                $receive_user = $this->Cms_admin_model->getUser($receiver_ids);
                if($receive_user->pm_sendmail == '1'){
                    ...
                    @$this->Cms_model->sendEmail($receive_user->email, '[PM] ' . $title . ' ('.$config->site_name.')', $message_html, $sender_user->email, $sender_user->name);
                }
                return TRUE;
            }else{
                return FALSE;
    ...
    public function list_pms($limit = 5, $offset = 0, $receiver_id = NULL, $sender_id = NULL, $unread = FALSE) {
        ...
        $rs = $this->Cms_admin_model->getIndexData('user_pms', $limit, $offset, 'id', 'desc', $search_sql);
        if($rs !== FALSE){
            return $rs;
        }else{
            return FALSE;
        ...
        if (!is_numeric($user_admin_id) || !is_numeric($pm_id)) {
            ...
            if ($user_admin_id == $result->sender_id) {
                if ($result->pm_deleted_receiver == 1) {
                    $this->Cms_admin_model->removeData('user_pms', array('id' => $pm_id));
                }
    ...
    public function cleanup_pms() {
        ...
        $this->Cms_admin_model->removeData('user_pms', "date_sent < '".$date_sent."'");
    }
    ...
    public function count_unread_pms($receiver_id = FALSE) {
        ...
        $count = $this->Cms_model->countData('user_pms', $data);
        if ($count !== FALSE && $count > 0) {
            return $count;
        }else{
            return 0;
