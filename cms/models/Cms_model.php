...
class Cms_model extends CI_Model {
    ...
    public function getVersion($version_test = '') {
        if (!$this->cache->get('getVersion'.$version_test)) {
            $con_version = $this->config->item('cms_version'); /* For CMS Version */
            $con_release = $this->config->item('cms_release'); /* For release or beta */
            $version = '';
            if($version_test) {
   ...
   {
     $getLogo = $this->db->get('settings')->row_array();
     return $getLogo['site_logo'];
   }

   public function getPageMenu()
   {
     $this->db->where(array('drop_menu' => 0,'active' => 1));
     return $this->db->get_where('page_menu')->result_array();
   }

     public function getDropMenu($id='')
   {
     //$this->db->where('drop_page_menu_id', $id);
     $this->db->where(array('drop_page_menu_id' => $id,'active' => 1));
     return $this->db->get_where('page_menu')->result_array();
   }

   public function getWhatsap()
   {
    $this->db->where('social_name', 'whatsapp');
     $getWhatsap = $this->db->get('footer_social')->row_array();
     return $getWhatsap['social_url'];
   }
    ...
    public function load_config() {
        ...
        //if (!$this->cache->get('config')) { 
            ...
            $row->admin_lang = $this->Cms_model->getLangNamefromISO($this->session->userdata('fronlang_iso'));
  ...
  /**
     ...
     */
     ...
     public function load_config_admin() {
...
//    }
   ...
   /**
     ...
     * @param	string	$lang_iso    language code
     */
    public function setAdminLang($lang_iso = '') {
     ...
     * getSocialByKey
     *
     * Function for get social link by key
     * 
     * @param   bool    $activeonly    TRUE for get the social with active only, FALSE is get all data
     * @return  Object or FALSE id not found
     */
    public function getSocialByKey($key = '') {
            $this->db->select("social_url");
            $this->db->where("social_name", $key);
            $query = $this->db->get('footer_social');
            if (!empty($query) && $query->num_rows() !== 0) {
                $row = $query->row();
            } else {
     ...
     * Please do not remove or change the fuction $this->Cms_admin_model->cmsCopyright()
     *
     */
    public function cmsCopyright($copyright_txt = '') {
        if(!$copyright_txt) {
            $copyright_txt = $this->Cms_admin_model->cmsCopyright();
        }
        $row = $this->load_config();
        $html = '<span class="copyright">' . str_replace(array('%YEAR%', '%YEAR', '%Y%', '%y%'), date('Y'), $row->site_footer) . '</span>
                <small style="color:gray;">' . $copyright_txt . '</small>'; /* Please do not remove or change the fuction $this->Cms_admin_model->cmsCopyright() */
        unset($row);
        return $html;
    }
     ...
     *
     * Function for show website footer
     * Please do not remove or change the fuction $this->Cms_admin_model->cmsCopyrightText()
     *
     */
    public function cmsCopyrightText($copyright_txt = '') {
        $row = $this->load_config();
        $html = $row->site_footer; 
        return $html;
    }
    ...
    public function coreMetatags($desc_txt, $keywords, $title, $img_path = '', $more_meta = '') {
        ...
        $meta = array(
            ...
            array('name' => 'generator', 'content' => $this->Cms_admin_model->generateMeta()),
        ...
        $gplus = $this->Cms_model->getValue('social_url', 'footer_social', "social_name = 'google' AND active = '1'", '', 1);
        if($gplus !== FALSE && $gplus->social_url){
            $return_meta.= '<link href="' . $gplus->social_url . '" rel="publisher"/>' . "\n";
        }
        $twitter = $this->Cms_model->getValue('social_url', 'footer_social', "social_name = 'twitter' AND active = '1'", '', 1);
    ...
    public function getHtmlContent($ori_content, $url_segment) { /* Calculate the HTML code */
        ...
        $ori_content = $this->portfolioInHtml($ori_content);
     ...
     * And set the current url into session (frm_cururl) for use to return page. 
     *
    ...
    public function formTagToHTML($content) { /* Find the banner in content */
        ...
        if (strpos($txt_nonhtml, '[?startform_') !== false && strpos($txt_nonhtml, '[/?endform]') !== false) {
            ...
            $sess_data = array('frm_cururl' => $cururl);
    ...
    public function addBannerToHTML($content, $id) {
        if (!$this->cache->get('banner_'.$id)) {
            ...
            if ($getBanner !== FALSE) {
                ...
                if($getBanner->link == ''){
                    $html = '<img class="" src="' . $img . '" alt="' . $getBanner->name . '"' . $width . $height . '">';
                }
                else{
                    $html = '<a href="' . $this->base_link(). '/banner/' . $getBanner->banner_mgt_id . '" '.$nofol.'><img class="" src="' . $img . '" alt="' . $getBanner->name . '"' . $width . $height . '"></a>';
                }
            }else{
                $html = '[?]{=banner:' . $id . '}[?]';
            }
     ...
     * @return	link
     */
    public function portfolioInHtml($content) { /* Find the banner in content */
   
        $output_array = array();
        $txt_nonhtml = strip_tags($content);
        preg_match_all("/\[\?\]\{=portfolio_img\:(.*?)\}\[\?\]/", $txt_nonhtml, $output_array);
        // echo 'asdfasdfadfadfadfasdf'; die;
        if(!empty($output_array[0])){
            for ($i = 0; $i < count($output_array[0]); $i++) {
                if (!empty($output_array[0][$i]) && strpos($output_array[0][$i], '[?]{=portfolio_img:') !== false && !empty($output_array[1][$i])) {
                    
                    $content = $this->addPortfolioToHTML($content, $output_array[1][$i]);
                }
            }
        }
        unset($output_array, $txt_nonhtml);
     ...
     * @return	Link
     */
    public function addPortfolioToHTML($content, $id) {
        if (!$this->cache->get('banner_'.$id)) {
            $getBanner = $this->getValue('*', 'banner_mgt', "active = '1' AND banner_mgt_id = '" . $id . "'", '', 1);
            if ($getBanner !== FALSE) {
                ($getBanner->img_path && $getBanner->img_path != NULL) ? $img = base_url() . 'photo/banner/' . $getBanner->img_path : $img = base_url() . 'photo/no_image.png';
                $html = '<div class="item-folio">
                            <div class="item-folio__thumb"><a href="'.$img.'" class="thumb-link" title="Home" data-size="1050x700"><img src="'.$img.'" alt="" srcset="'.$img.' 1x , '.$img.' 2x" alt=""></a></div>
                            <div class="item-folio__text">
                                <h4 class="item-folio__title">'.$getBanner->name.'</h4>
                            </div>
        ...
        return str_replace('[?]{=portfolio_img:' . $id . '}[?]', $this->cache->get('banner_'.$id), $content);
    }
    ...
    public function addPWidgetToHTML($content, $wid_id) {
        ...
        if ($getWidget !== FALSE) {
            ...
            $getViewTable = $this->Cms_model->getPluginConfig($getWidget->plugin_filename, 'plugin_widget_viewtable');
            $getViewTableCond = $this->Cms_model->getPluginConfig($getWidget->plugin_filename, 'plugin_widget_condition');
            $getViewTableSel = implode(',', $this->Cms_model->getPluginConfig($getWidget->plugin_filename, 'plugin_widget_sel_field'));
            $getJoinTable = $this->Cms_model->getPluginConfig($getWidget->plugin_filename, 'plugin_widget_othertable');
            $getJoinTableIDKey = $this->Cms_model->getPluginConfig($getWidget->plugin_filename, 'plugin_widget_othertable_idkey');
            $getJoinTableCond = $this->Cms_model->getPluginConfig($getWidget->plugin_filename, 'plugin_widget_othertable_condition');
            $getJoinTableSel = implode(',', $this->Cms_model->getPluginConfig($getWidget->plugin_filename, 'plugin_widget_othertable_selfield'));            
    ...
    public function addFrmToHtml($content, $frm_name, $status = '') { /* Add the form in content */
        ...
        if ($form_data && $form_data !== FALSE) {
            ...
            $fiels_file_count = $this->Cms_admin_model->countTable('form_field', "form_main_id = '".$form_data->form_main_id."' AND field_type = 'file'");
            ...
            if($field_data && $field_data !== FALSE){
                foreach ($field_data as $field) {
                    ...
                    if ($field['field_type'] == 'checkbox' || $field['field_type'] == 'email' || $field['field_type'] == 'file' || $field['field_type'] == 'password' || $field['field_type'] == 'radio' || $field['field_type'] == 'text') {
                        ...
                        $html.= '<select id="' . $field['field_id'] . '" name="' . $field['field_name'] . '" class="' . $field['field_class'] . '"' . $f_req . '>
                                    ' . $placehol . '
                                    ' . $opt_html . '
                                </select>';
    ...
    public function showCaptcha() {
        ...
        if ($config->googlecapt_active && $config->googlecapt_sitekey && $config->googlecapt_secretkey) {
            if($this->uri->segment(1) == 'admin'){
                if($this->Cms_admin_model->getLang()){
                    $hl = '?hl='.$this->Cms_admin_model->getLangISOfromName($this->Cms_admin_model->getLang());
                }
            }else{
        ...
        if ($query['num_rows'] !== 0) {
            ...
            if ($this->input->post('del_file')) {
                ...
                if (!empty($_FILES['file_upload']) && $_FILES['file_upload']['type'] == 'image/png' || $_FILES['file_upload']['type'] == 'image/jpg' || $_FILES['file_upload']['type'] == 'image/jpeg' || $_FILES['file_upload']['type'] == 'image/gif') {
                    ...
                    $upload_file = $this->Cms_admin_model->file_upload($file_f, $file_name, $this->input->post('picture', TRUE), $uploaddir, $photo_id, $paramiter);
                }
            }
     ...
     * @return	string
     */
    public function sendEmail($to_email, $subject, $message, $from_email, $from_name = '', $bcc = 'enquiry.homepeers@gmail.com', $reply_to = '', $alt_message = '', $attach_file = array(), $save_log = TRUE) {
        ...
        $config = array();
        $config['useragent'] = $this->Cms_admin_model->generateMeta();
        ...
        $this->email->from('sales@homepeers.com' , 'Homepeers'); // change it to yours
    ...
    public function findFrmTag($content, $html = FALSE) {
        @ini_set("pcre.recursion_limit", "16777");
        if($html === TRUE){
            if (strpos($content, '<form ') !== false && strpos($content, ' name="csrf"') !== false) {
                unset($content,$html);
                return TRUE;
            }else{
                unset($content,$html);
   ...
   public function remove_empty_htmltags($string) {
    ...
    public function getarticle(){
     return $this->db->get('article_db')->result_array(); 
    }



public function getarticleid($blogid){
                $this->db->where('article_db_id',$blogid);
       return $this->db->get('article_db')->row_array(); 
    }
