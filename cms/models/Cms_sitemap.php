...
class Cms_sitemap extends CI_Model {
    ...
    function __construct() {
        ...
        $this->lang = $this->Cms_model->getValueArray('lang_iso', 'lang_iso', 'active', 1, 0, 'lang_iso_id', 'ASC');
        $this->menu_other = $this->Cms_model->getValueArray('*', 'page_menu', "active = '1' AND drop_menu != '1'", '', 0, 'menu_name', 'ASC');
        $this->pages_content = $this->Cms_model->getValueArray('*', 'pages', "active = '1'", '', 0, 'page_url', 'ASC');
    ...
    private function genRobotTXT() {
        ...
        $robots_txt = '# robots.txt generated by CMS'."\n";
    ...
    private function genSitemapXML() {
        ...
        <!-- created by CMS Sitemap Generator-->'."\n";
        $sitemap_xml.= '<url>
	<loc>'.base_url().'</loc>
	<changefreq>always</changefreq>
        ...
        if($this->lang !== FALSE){ /* Language */
            foreach ($this->lang as $row) {
                $url = $this->Cms_model->base_link().'/lang/'.$row['lang_iso'];
        ...
        if($this->menu_other !== FALSE){ /* Navigation */
            foreach ($this->menu_other as $row) {
                ...
                if($row['pages_id'] && $row['pages_id'] != NULL && $row['pages_id'] != 0){
                    $pages = $this->Cms_model->getValue('page_url', 'pages', "active = '1' AND pages_id = '".$row['pages_id']."'", '', 1, 'page_url', 'ASC');                    
                    if($row['drop_page_menu_id'] != 0 && $row['drop_page_menu_id'] != NULL){
                        $main = $this->Cms_model->getValue('menu_name', 'page_menu', "active = '1' AND page_menu_id = '".$row['drop_page_menu_id']."'", '', 1, 'menu_name', 'ASC');                    
                        $url = $this->Cms_model->base_link().'/'.$this->Cms_model->rw_link($main->menu_name).'/'.$pages->page_url;
                        if(!in_array($url, $url_same)){
                            $sitemap_xml.= '<url>
                            <loc>'.$url.'</loc>
                            <changefreq>always</changefreq>
                        ...
                        $url = $this->Cms_model->base_link().'/'.$pages->page_url;
                        if(!in_array($url, $url_same)){
                            $sitemap_xml.= '<url>
                            <loc>'.$url.'</loc>
                            <changefreq>always</changefreq>
                    ...
                    $url = $this->Cms_model->base_link().'/plugin/'.$row['plugin_menu'];
                    if(!in_array($url, $url_same)){
                        $sitemap_xml.= '<url>
                        <loc>'.$url.'</loc>
                        <changefreq>always</changefreq>
        ...
        if($this->pages_content !== FALSE){ /* Pages Content without navigation */
            foreach ($this->pages_content as $row) {
                $url = $this->Cms_model->base_link().'/'.$row['page_url'];
                if(!in_array($url, $url_same)){
                    $sitemap_xml.= '<url>
                    <loc>'.$url.'</loc>
                    <changefreq>always</changefreq>
        ...
        if($this->plugin !== FALSE){ /* Plugin with sitemap config */
            foreach ($this->plugin as $row) {
                $plugin_db = $this->Cms_model->getPluginConfig($row['plugin_config_filename'], 'plugin_sitemap_viewtable');
                if(!empty($plugin_db)){
                    $plugindata = $this->Cms_model->getValueArray('*', $plugin_db, $this->Cms_model->getPluginConfig($row['plugin_config_filename'], 'plugin_sqlextra_condition'), '', 0, $plugin_db.'_id', 'DESC');
                    if($plugindata !== FALSE){
                        foreach ($plugindata as $rs) {
                            $url = $this->Cms_model->base_link().'/plugin/'.$this->Cms_model->getPluginConfig($row['plugin_config_filename'], 'plugin_urlrewrite').'/view/'.$rs[$plugin_db.'_id'].'/'.$rs['url_rewrite'];
                            if(!in_array($url, $url_same)){
                                $sitemap_xml.= '<url>
                                <loc>'.$url.'</loc>
                                <changefreq>always</changefreq>
                            ...
                            if($row['plugin_config_filename'] == 'article'){
                                $urlamp = $this->Cms_model->base_link().'/plugin/'.$this->Cms_model->getPluginConfig($row['plugin_config_filename'], 'plugin_urlrewrite').'/amp/'.$rs[$plugin_db.'_id'].'/'.$rs['url_rewrite'];
                ...
                $plugin_cat_db = $this->Cms_model->getPluginConfig($row['plugin_config_filename'], 'plugin_sitemap_cattable');
                if(!empty($plugin_cat_db)){
                    $plugindata = $this->Cms_model->getValueArray('*', $plugin_cat_db, $this->Cms_model->getPluginConfig($row['plugin_config_filename'], 'plugin_sqlextra_catcondition'), '', 0, $plugin_cat_db.'_id', 'DESC');
                    if($plugindata !== FALSE){
                        foreach ($plugindata as $rs) {
                            $url = $this->Cms_model->base_link().'/plugin/'.$this->Cms_model->getPluginConfig($row['plugin_config_filename'], 'plugin_urlrewrite').'/category/'.$rs['url_rewrite'];
                            if(!in_array($url, $url_same)){
                                $sitemap_xml.= '<url>
                                <loc>'.$url.'</loc>
                                <changefreq>always</changefreq>
	...
	<ror:resourceOf>sitemap</ror:resourceOf>
        ...
        if($this->menu_other !== FALSE){ /* Navigation */
            ...
            foreach ($this->menu_other as $row) {
                ...
                $chkotherlink = strpos($row['other_link'], BASE_URL);
                if($row['pages_id'] && $row['pages_id'] != NULL && $row['pages_id'] != 0){
                    $pages = $this->Cms_model->getValue('*', 'pages', "active = '1' AND pages_id = '".$row['pages_id']."'", '', 1, 'page_url', 'ASC');                    
                    if($row['drop_page_menu_id'] != 0 && $row['drop_page_menu_id'] != NULL){
                        $main = $this->Cms_model->getValue('menu_name', 'page_menu', "active = '1' AND page_menu_id = '".$row['drop_page_menu_id']."'", '', 1, 'menu_name', 'ASC');                    
                        $url = $this->Cms_model->base_link().'/'.$this->Cms_model->rw_link($main->menu_name).'/'.$pages->page_url;
                        if(!in_array($url, $url_same)){
    ...
    private function genSitemapHTML() {
        ...
        <html>
        <head>
        <title>HTML Site Map - Generated by CMS</title>
        <meta http-equiv="Content-type" content="text/html; charset=utf-8" />
        <meta name="viewport" content="width=device-width" />
        </head>
        <body>
        <div>
        <h1>HTML Site Map by <a href="#" target="_blank"> CMS '.$this->Cms_model->getVersion().'</a></h1>
