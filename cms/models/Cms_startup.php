...
class Cms_startup extends CI_Model {
    ...
    function __construct() {
        ...
        $this->plugin = $this->Cms_model->getValueArray('plugin_config_filename', 'plugin_manager', "plugin_active = '1' AND plugin_config_filename != ''", '', 0, 'timestamp_update', 'DESC');
    }
        ...
        if (!$this->cache->get('startupPluginBackend')) {
            ...
            if(!empty($this->plugin)){
                foreach ($this->plugin as $value) {
                    $config = $this->Cms_model->getPluginConfig($value['plugin_config_filename'], 'backend_startup');
                    $base_url = $this->Cms_model->base_link().'/admin/plugin/'.$this->Cms_model->getPluginConfig($value['plugin_config_filename'], 'plugin_urlrewrite').'/';
                    if($config !== FALSE && $base_url !== FALSE){
                        if($config) $return[] = $base_url.$config;
                    }
                }
            }
            ($this->Cms_model->load_config()->pagecache_time == 0) ? $cache_time = 1 : $cache_time = $this->Cms_model->load_config()->pagecache_time;
    ...
    public function getPluginFrontend() {
        if (!$this->cache->get('startupPluginFrontend')) {
            ...
            if(!empty($this->plugin)){
                foreach ($this->plugin as $value) {
                    $config = $this->Cms_model->getPluginConfig($value['plugin_config_filename'], 'frontend_startup');
                    $base_url = $this->Cms_model->base_link().'/plugin/'.$this->Cms_model->getPluginConfig($value['plugin_config_filename'], 'plugin_urlrewrite').'/';
                    if($config !== FALSE && $base_url !== FALSE){
                        if($config) $return[] = $base_url.$config;
                    }
                }
