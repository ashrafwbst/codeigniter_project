...
class Gallery_model extends CI_Model {
    ...
    public function insert() {
        // Create the new lang
        $arrange = $this->Cms_model->getLastID('gallery_db', 'arrange');
        ($this->input->post('active')) ? $active = $this->input->post('active', TRUE) : $active = 0;
        $url_rewrite = $this->Cms_model->rw_link($this->input->post('album_name', TRUE));
    ...
    public function delete($id) {
        if($id){
            $this->Cms_admin_model->removeData('gallery_db', 'gallery_db_id', $id);
        }else{
            return FALSE;
        }
    }
    ...
    public function insertFileUpload($gallery_db_id, $gallery_type, $fileupload = '', $youtube_url = '') {
        $img_rs = $this->Cms_model->getValue('arrange', 'gallery_picture', "gallery_db_id", $gallery_db_id, 1, 'arrange', 'desc');
        if(!empty($img_rs)){
    ...
    public function getFirstImgs($gallery_db_id) {
        ...
        if($gallery_db_id){
            $img_rs = $this->Cms_model->getValue('file_upload,gallery_type,youtube_url', 'gallery_picture', "gallery_db_id", $gallery_db_id, 1, 'arrange', 'asc');
            if(!empty($img_rs)){
