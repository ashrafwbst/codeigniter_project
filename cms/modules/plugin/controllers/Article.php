...
class Article extends CI_Controller {
    ...
    public function index() {
        $row = $this->Cms_model->load_config();
        $title = $this->Cms_model->getLabelLang('article_index_header') . ' | ' . $row->site_name;
        ...
        // Pages variable
        $result_per_page = 10;
        $total_row = $this->Cms_model->countData('article_db', $search_arr);
        $num_link = 10;
        $base_url = $this->Cms_model->base_link(). '/plugin/article/';

        // Pageination config
        $this->Cms_admin_model->pageSetting($base_url, $total_row, $result_per_page, $num_link, 3);
        ($this->uri->segment(3)) ? $pagination = $this->uri->segment(3) : $pagination = 0;

        //Get users from database
        $this->template->setSub('article', $this->Cms_admin_model->getIndexData('article_db', $result_per_page, $pagination, 'timestamp_create', 'desc', $search_arr));
    ...
    public function category() {
        if($this->uri->segment(4)){
            if($this->uri->segment(5)){
                $maincat_row = $this->Cms_model->getValue('article_db_id', 'article_db', "is_category = '1' AND active = '1' AND url_rewrite = '".$this->uri->segment(4)."'", '', 1);
                if($maincat_row !== FALSE){
                    $cat_row = $this->Cms_model->getValue('*', 'article_db', "is_category = '1' AND active = '1' AND main_cat_id = '".$maincat_row->article_db_id."' AND url_rewrite = '".$this->uri->segment(5)."'", '', 1);
                }else{
                    $cat_row = FALSE;
                }
                unset($maincat_row);
            }else{
                $cat_row = $this->Cms_model->getValue('*', 'article_db', "is_category = '1' AND active = '1' AND url_rewrite = '".$this->uri->segment(4)."'", '', 1);
            }
            if($cat_row !== FALSE){
                if($this->session->userdata('fronlang_iso') != $cat_row->lang_iso){
                    $this->Cms_model->setSiteLang($cat_row->lang_iso);
                }
                $row = $this->Cms_model->load_config();
                $title = $this->Cms_model->getLabelLang('article_index_header') . ' | ' . $row->site_name;
                $this->template->set('title', $title);
                ...
                $num_link = 10;
                if($this->uri->segment(5)){
                    $base_url = $this->Cms_model->base_link(). '/plugin/article/category/'.$this->uri->segment(4).'/'.$this->uri->segment(5).'/';

                    // Pageination config
                    $this->Cms_admin_model->pageSetting($base_url, $total_row, $result_per_page, $num_link, 6);
                    ...
                    $base_url = $this->Cms_model->base_link(). '/plugin/article/category/'.$this->uri->segment(4).'/';
                ...
                redirect($this->Cms_model->base_link().'/plugin/article', 'refresh');
            }
        }else{
            redirect($this->Cms_model->base_link().'/plugin/article', 'refresh');
        }
    ...
    public function search() {
        $p = $this->Cms_model->cleanOSCommand($this->input->get('p', TRUE));
        if ($p) {
            $row = $this->Cms_model->load_config();
            ...
            $total_row = $this->Cms_model->countData('article_db', $search_arr);
            $num_link = 10;
            $base_url = $this->Cms_model->base_link(). '/plugin/article/search/';

            // Pageination config
            $this->Cms_admin_model->pageSetting($base_url, $total_row, $result_per_page, $num_link, 4);
    ...
    public function archive() {
        if($this->uri->segment(4)){
            ...
            if($year_arr !== FALSE){
                ...
                $total_row = $this->Cms_model->countData('article_db', $search_arr);
                $num_link = 10;
                $base_url = $this->Cms_model->base_link(). '/plugin/article/archive/'.$this->uri->segment(4).'/';
    ...
    public function view() {
        if($this->uri->segment(4) && $this->uri->segment(5)){
            $art_row = $this->Cms_model->getValue('*', 'article_db', "is_category = '0' AND active = '1' AND article_db_id = '".$this->uri->segment(4)."'", '', 1);
            if($art_row !== FALSE){
                if($this->session->userdata('fronlang_iso') != $art_row->lang_iso){
                    $this->Cms_model->setSiteLang($art_row->lang_iso);
                }
                $row = $this->Cms_model->load_config();
                if ($this->Cms_model->findFrmTag($art_row->content) !== false) {
                    $row->pagecache_time = 0;
                    $this->db->cache_off();
                }
                $this->output->cache($row->pagecache_time);
                $this->template->set('title', $art_row->title.' | ' . $row->site_name);
                $more_meta = '<link rel="amphtml" href="'.$this->Cms_model->base_link().'/plugin/article/amp/'.$this->uri->segment(4).'/'.$this->uri->segment(5).'/" />' . "\n";
                ...
                $facebook = $this->Cms_model->getValue('*', 'footer_social', "social_name = 'facebook' AND active = '1'", '', 1);
                ...
                $more_meta.= '<script type="application/ld+json">
                {
                    ...
                    "mainEntityOfPage": {
                        "@type": "WebPage",
                        "@id": "' . $this->Cms_model->base_link() . '/plugin/article/view/' . $this->uri->segment(4) . '/' . $this->uri->segment(5) . '"
                    },
                    ...
                    "author": {
                        "@type": "Person",
                        "name": "' . ucfirst($this->Cms_admin_model->getUser($art_row->user_admin_id)->name) . '"
                    },
                    "publisher": {
                        "@type": "Organization",
                        "name": "' . $row->site_name . '",
                ...
                $this->template->set('meta_tags', $this->Cms_model->coreMetatags($art_row->short_desc,$art_row->keyword,$art_row->title,'plugin/article/' . $art_row->main_picture,$more_meta));
                $this->template->set('cur_page', $this->page_url);

                //Get users from database
                $this->template->setSub('article', $art_row);
                $cat_row = $this->Cms_model->getValue('*', 'article_db', "is_category = '1' AND active = '1' AND article_db_id = '".$art_row->cat_id."'", '', 1);                
                if($cat_row->main_cat_id != '0' && $cat_row->main_cat_id != NULL){
                    $main_cat_name = $this->Article_model->getCatNameFromID($cat_row->main_cat_id);
                    $this->template->setSub('category_name', '<a href="' . $this->Cms_model->base_link().'/plugin/article/category/' . $this->Cms_model->rw_link($main_cat_name) . '/' . $this->Cms_model->rw_link($cat_row->category_name) . '" title="' . $main_cat_name . ' -> ' . $cat_row->category_name.'">' . $main_cat_name . ' -> ' . $cat_row->category_name.'</a>');
                }else{
                    $this->template->setSub('category_name', '<a href="' . $this->Cms_model->base_link().'/plugin/article/category/' . $this->Cms_model->rw_link($cat_row->category_name) . '" title="' . $cat_row->category_name . '">' . $cat_row->category_name . '</a>');
                }
                //Load the view
    ...
    public function amp() {
        if($this->uri->segment(4) && $this->uri->segment(5)){
            ...
            if($art_row !== FALSE){
                ...
                $this->output->cache($row->pagecache_time);
                $this->template->set('title', $art_row->title.' | ' . $row->site_name);
                $canonical = '<link rel="canonical" href="'.$this->Cms_model->base_link().'/plugin/article/view/'.$this->uri->segment(4).'/'.$this->uri->segment(5).'">' . "\n";               
                ...
                $schematag = '<script type="application/ld+json">
                {
                    ...
                    "mainEntityOfPage": {
                        "@type": "WebPage",
                        "@id": "' . $this->Cms_model->base_link() . '/plugin/article/amp/' . $this->uri->segment(4) . '/' . $this->uri->segment(5) . '"
                    },
                    "headline": "' . $art_row->title . '",
                    "image": {
                        "@type": "ImageObject",
    ...
    public function markupview() {
        if($this->uri->segment(4)){
            ...
            if($art_row !== FALSE){
                ...
                $this->template->setSub('canonical', '<link rel="canonical" href="'.$this->Cms_model->base_link().'/plugin/article/markupview/'.$this->uri->segment(4).'">' . "\n");
                $this->template->setSub('article', $art_row);
                $cat_row = $this->Cms_model->getValue('*', 'article_db', "is_category = '1' AND active = '1' AND article_db_id = '".$art_row->cat_id."'", '', 1);                
                if($cat_row->main_cat_id != '0' && $cat_row->main_cat_id != NULL){
                    $main_cat_name = $this->Article_model->getCatNameFromID($cat_row->main_cat_id);
                    $category_name = '<a href="' . $this->Cms_model->base_link().'/plugin/article/category/' . $this->Cms_model->rw_link($main_cat_name) . '/' . $this->Cms_model->rw_link($cat_row->category_name) . '" title="' . $main_cat_name . ' -> ' . $cat_row->category_name.'">' . $main_cat_name . ' -> ' . $cat_row->category_name.'</a>';
                }else{
                    $category_name = '<a href="' . $this->Cms_model->base_link().'/plugin/article/category/' . $this->Cms_model->rw_link($cat_row->category_name) . '" title="' . $cat_row->category_name . '">' . $cat_row->category_name . '</a>';
                }
    ...
    public function rss() {
        ...
        $feed->description = $this->Cms_model->getLabelLang('article_index_header') . ' | ' . $row->site_name;
        $feed->link = $this->Cms_model->base_link().'/plugin/article';
        $search_arr = " is_category = '0' AND active = '1'";
        $limit = 20;
        // get article list
        $article = $this->Cms_admin_model->getIndexData('article_db', $limit, 0, 'timestamp_create', 'desc', $search_arr);
        // add posts to the feed
        if($article !== FALSE){
            foreach ($article as $a)
            {
		$content = $this->Cms_model->get_contents_url($this->Cms_model->base_link().'/plugin/article/markupview/'.$a['article_db_id']);
                // set item's title, author, url, pubdate and description
                $url = $this->Cms_model->base_link().'/plugin/article/view/'.$a['article_db_id'].'/'.$a['url_rewrite'];
    ...
    public function getWidget() {
        ...
        if (!$this->cache->get('article_getWidget_'.$this->uri->segment(4).'_'.$this->uri->segment(5))) {
            ...
            // Initiate class
            $xml = new Xml_writer;
            $xml->setRootName('cms_widget');
            $xml->initiate();
            // Start Main branch
            $xml->startBranch('plugin'); 
            $xml->addNode('main_url', $this->Cms_model->base_link().'/plugin/article');
            ...
            $article = $this->Cms_admin_model->getIndexData('article_db', 100, 0, 'timestamp_create', 'desc', $search_arr);
            if($article !== FALSE){
                $xml->addNode('null', '0'); // For check item is not empty
                foreach ($article as $row) {
                    // start sub branch
                    $xml->startBranch('item', array('id' => $row['article_db_id'])); 
                    $xml->addNode('sub_url', $this->Cms_model->base_link().'/plugin/article/view/'.$row['article_db_id'].'/'.$row['url_rewrite']);
                    $xml->addNode('title', $row['title']);
                    $xml->addNode('short_desc', $row['short_desc']);
                    if($row['main_picture']){
                        $xml->addNode('photo', $this->Cms_model->base_link().'/photo/plugin/article/'.$row['main_picture']);
                    }else{
                        $xml->addNode('photo', $this->Cms_model->base_link().'/photo/no_image.png');
                    }
                    // End sub branch
                    $xml->endBranch();
                }
