...
class Gallery extends CI_Controller {
    ...
    public function _init() {
        $this->template->set('core_css', $this->Cms_model->coreCss('assets/css/ekko-lightbox.min.css'));
        $js_arr = array(base_url() . 'assets/js/ekko-lightbox.min.js', base_url() . 'assets/js/ekko-lightbox.run.js');
        $this->template->set('core_js', $this->Cms_model->coreJs($js_arr));
        $row = $this->Cms_model->load_config();
    ...
    public function index() {
        $row = $this->Cms_model->load_config();
        $title = $this->Cms_model->getLabelLang('gallery_header') . ' | ' . $row->site_name;
        ...
        // Pages variable
        $result_per_page = 15;
        $total_row = $this->Cms_model->countData('gallery_db', $search_arr);
        $num_link = 10;
        $base_url = $this->Cms_model->base_link(). '/plugin/gallery/';
        ...
        $this->template->setSub('gallery', $this->Cms_admin_model->getIndexData('gallery_db', $result_per_page, $pagination, 'arrange', 'asc', $search_arr));
    ...
    public function view() {
        if($this->uri->segment(4)){
            $album_row = $this->Cms_model->getValue('*', 'gallery_db', "active = '1' AND gallery_db_id = '".$this->uri->segment(4)."'", '', 1);
            if($album_row !== FALSE){
                if($this->session->userdata('fronlang_iso') != $album_row->lang_iso){
                    $this->Cms_model->setSiteLang($album_row->lang_iso);
                }
                $row = $this->Cms_model->load_config();
                $this->template->set('title', $album_row->album_name.' | ' . $row->site_name);
                $this->template->set('meta_tags', $this->Cms_model->coreMetatags($album_row->short_desc,$album_row->keyword,$album_row->album_name,$this->Gallery_model->getFirstImgs($album_row->gallery_db_id)));
                $this->template->set('cur_page', $this->page_url);
                ...
                $result_per_page = 30;
                $total_row = $this->Cms_model->countData('gallery_picture', $search_arr);
                $num_link = 10;
                $base_url = $this->Cms_model->base_link(). '/plugin/gallery/view/'.$this->uri->segment(4).'/'.$album_row->url_rewrite.'/';
                ...
                //Get users from database
                $this->template->setSub('image', $this->Cms_admin_model->getIndexData('gallery_picture', $result_per_page, $pagination, 'arrange', 'asc', $search_arr));
                $this->template->setSub('total_row', $total_row);

                //Load the view
                $this->template->loadFrontPlugin('gallery/gallery_view');
            }else{
                redirect($this->Cms_model->base_link().'/plugin/gallery', 'refresh');
            }
        }else{
            redirect($this->Cms_model->base_link().'/plugin/gallery', 'refresh');
        }
    ...
    public function viewxml() {
        ...
        if (!$this->cache->get('gallery_getWidget_album' . $this->uri->segment(4))) {
            ...
            $album_row = $this->Cms_model->getValue('*', 'gallery_db', "active = '1' AND gallery_db_id = '" . $this->uri->segment(4) . "'", '', 1);
            if($album_row === FALSE){
                $xml->addNode('main_url', $this->Cms_model->base_link().'/plugin/gallery');
                $xml->addNode('null', '1');
            }else{
                $xml->addNode('main_url', $this->Cms_model->base_link() . '/plugin/gallery/view/' . $album_row->gallery_db_id . '/' . $album_row->url_rewrite);
                $search_arr = "gallery_db_id = '" . $album_row->gallery_db_id . "'";
                $photo = $this->Cms_admin_model->getIndexData('gallery_picture', 100, 0, 'arrange', 'asc', $search_arr);
                if ($photo !== FALSE) {
    ...
    public function rss() {
        ...
        $feed->description = $this->Cms_model->getLabelLang('gallery_header') . ' | ' . $row->site_name;
        $feed->link = $this->Cms_model->base_link().'/plugin/gallery';
        $search_arr = " active = '1'";
        $limit = 20;
        // get article list
        $gallery = $this->Cms_admin_model->getIndexData('gallery_db', $limit, 0, 'arrange', 'asc', $search_arr);
        // add posts to the feed
        if($gallery !== FALSE){
            foreach ($gallery as $a)
            {
                // set item's title, author, url, pubdate and description
                $url = $this->Cms_model->base_link().'/plugin/gallery/view/'.$a['gallery_db_id'].'/'.$a['url_rewrite'];
    ...
    public function getWidget() {
        ...
        if (!$this->cache->get('gallery_getWidget_'.$this->uri->segment(4))) {
            ...
            $article = $this->Cms_admin_model->getIndexData('gallery_db', 100, 0, 'arrange', 'asc', $search_arr);
            if($article !== FALSE){
                $xml->addNode('null', '0'); // For check item is not empty
                foreach ($article as $row) {
                    // start sub branch
                    $xml->startBranch('item', array('id' => $row['gallery_db_id'])); 
                    $xml->addNode('sub_url', $this->Cms_model->base_link().'/plugin/gallery/view/'.$row['gallery_db_id'].'/'.$row['url_rewrite']);
