...
<div class="row">
    <div class="col-lg-12 col-md-12">
        <div class="h2 sub-header"><?php echo $form_name?>  <a role="button" href="<?php echo $this->cms_referrer->getIndex()?>" class="btn btn-default btn-sm"><i class="glyphicon glyphicon-arrow-left"></i> <?php echo  $this->lang->line('btn_back') ?></a></div>
        <?php echo  form_open($this->Cms_model->base_link().'/admin/forms/view/'.$this->uri->segment(4).'/delete'); ?>
                                ...
                                foreach ($field_rs as $field) { 
                                    ...
                                    <?php }else if($field['field_type'] == 'file'){ 
                                        if($u[$field['field_name']]){
                                            $linkhtml = '<a href="' . base_url() . "photo/forms/" . $this->Cms_model->cleanEmailFormat($form_name) . '/' . $this->Cms_model->cleanEmailFormat($field['field_name']) . '/' . $u[$field['field_name']] . '" target="_blank">' . $u[$field['field_name']] . '</a>';
                                        }else{
                                            $linkhtml = '-';
                                        } ?>
