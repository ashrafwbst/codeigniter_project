...
<div class="row">
    <div class="col-lg-12 col-md-12">
        <div class="h2 sub-header"><?php echo  $this->lang->line('linkstats_header') ?> <a role="button" href="<?php echo $this->Cms_model->base_link()?>/admin/linkstats/new" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-plus"></span> <?php echo  $this->lang->line('linkstats_newbtn') ?></a></div>
        <form action="<?php echo $this->Cms_model->base_link(). '/admin/linkstats/'; ?>" method="get">
        ...
        <br><br>
        <?php echo  form_open($this->Cms_model->base_link(). '/admin/linkstats/deleteindexurl'); ?>
                            ...
                            <td style="vertical-align:middle;">
                                <div class="form-group has-warning">
                                    <div class="input-group">
                                        ...
                                        <input type="text" readonly class="form-control" id="full_url" value="<?php echo $this->Cms_model->base_link() ?>/link/<?php echo $u['link_stat_mgt_id'] ?>#<?php echo $u['url'] ?>" onfocus="this.select();" onmouseup="return false;">
                                    </div>
                                </div>
                            </td>
                            <?php $where_arr = array('link'=>$u['url']);
                            echo '<td class="text-center" style="vertical-align:middle;">' . number_format($this->Cms_model->countData('link_statistic', $where_arr)) . '</td>';
                            echo '<td class="text-center" style="vertical-align:middle;">' . $u['timestamp_create'] . '</td>';
                            echo '<td class="text-center" style="vertical-align:middle;"><a href="'.$this->Cms_model->base_link().'/admin/linkstats/view/' . $u['link_stat_mgt_id'] . '" class="btn btn-primary btn-sm" role="button"><i class="glyphicon glyphicon-eye-open"></i>  '.$this->lang->line('btn_view').'</a>';
                            echo '</td></tr>';
                        }
                        ?>
                    <?php } ?>
