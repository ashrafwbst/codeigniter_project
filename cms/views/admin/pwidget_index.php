...
<div class="row">
    <div class="col-lg-12 col-md-12">
        <div class="h2 sub-header"><?php echo $this->lang->line('pwidget_header') ?> <a role="button" href="<?php echo $this->Cms_model->base_link()?>/admin/pwidget/addWidget" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-plus"></span> <?php echo $this->lang->line('pwidget_new_header') ?></a></div>
                    ...
                    <?php } else { ?>
                        ...
                        foreach ($widget as $u) {
                            ...
                            if($u['lang_iso'] && $u['lang_iso'] != NULL){
                                $lang_flag = '<i class="flag-icon flag-icon-'.$this->Cms_model->getCountryCode($u['lang_iso']).'"></i>';
                            }else{
                                $lang_flag = $this->lang->line('option_all');
                            }
                            echo '<td'.$inactive.' class="text-center">'.$lang_flag.'</td>';
                            echo '<td'.$inactive.' class="text-center"><a onclick="return confirm(\''.$this->lang->line('delete_message').'\')"  href="'.$this->Cms_model->base_link().'/admin/pwidget/asCopy/' . $u['plugin_widget_id'] . '" class="btn btn-default btn-sm" role="button"><i class="glyphicon glyphicon-duplicate"></i>  '.$this->lang->line('btn_ascopy').'</a> &nbsp;&nbsp; <a href="'.$this->Cms_model->base_link().'/admin/pwidget/editWidget/' . $u['plugin_widget_id'] . '" class="btn btn-default btn-sm" role="button" title="'.$this->lang->line('btn_edit').'"><i class="glyphicon glyphicon-pencil"></i></a> &nbsp;&nbsp;&nbsp; <a role="button" class="btn btn-danger btn-sm" role="button" onclick="return confirm(\''.$this->lang->line('delete_message').'\')" href="'.$this->Cms_model->base_link().'/admin/pwidget/delete/'.$u['plugin_widget_id'].'" title="'.$this->lang->line('btn_delete').'"><i class="glyphicon glyphicon-remove"></i></a></td>';
                            echo '</tr>';
                        }
                    }    ?>
                </tbody>
